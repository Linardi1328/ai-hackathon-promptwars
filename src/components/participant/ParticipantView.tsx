...
import { Users, CheckCircle2, Clock, Hourglass, MapPin, Sparkles } from 'lucide-react';
          ...
          >
            {teams.map((t) => (
              <option key={t.id} value={t.id} className="bg-slate-900 text-white">
                {t.name} (Table #{t.tableNumber}) {t.checkedIn ? '✓' : '○'}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="md:col-span-4 bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
              ...
              {currentTeam.checkedIn ? (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                  <CheckCircle2 className="w-3 h-3" /> Checked In {currentTeam.checkedInAt ? `(${currentTeam.checkedInAt})` : ''}
                </span>
              ) : (
            ...
            <div className="space-y-1 text-center mt-3">
              ...
              <p className="text-xs text-emerald-400 font-mono">Table #{currentTeam.tableNumber} • {currentTeam.track}</p>
            </div>
